...
async function didToJs(candid_source, canisterId) {
...
async function getRemoteDidJs(canisterId) {
    if (!canisterId) {
    ...
    return didToJs(candid_source, canisterId);
}
...
export async function getActorFromCanisterId(canisterId) {
    if (!canisterId) {
        throw 'can not fetch actor for null canister id';
    }
    let js;
